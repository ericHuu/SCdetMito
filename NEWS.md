...
- Updated README, vignettes, examples, and pkgdown documentation for SoftwareX
  readiness.
- Prepared v1.4.1 as the recommended SoftwareX submission version.
...
- `devtools::test()`: 0 failures, 0 warnings, 8 skipped optional/local tests,
  211 passed expectations.
...
- Fixed and improved IFNB online loader for multi-condition QC examples,
  including optional `SeuratData::InstallData("ifnb")` support.
- Added robust mitochondrial feature detection fallback for IFNB and online
  examples.
...
- Clarified that PBMC3K is a small public-data check, heart10k is a
  solid-tissue stress-test candidate, mouse brain 5k is a tissue/nuclei
  scouting candidate, and IFNB is a multi-condition workflow example.
