...
#' the first significant high-to-low boundary, the largest-drop boundary, and
#' the selected cutoff under the requested decision mode. When species/tissue or
#' a user reference is supplied, literature-informed reference cutoffs are added
#' for interpretation and warning generation. A sample-supported global cutoff
#' is returned for multi-sample inputs.
...
#'   `selected_cutoff` is the v1.4.0 reference-aware decision field and
#'   `detected_cutoff` is retained as a backward-compatible alias. The
    ...
    for (marker_type in names(cutoff_line_types)) {
      ...
      plot_obj <- plot_obj +
        ggplot2::geom_vline(
          ...
          alpha = alpha_value,
          linewidth = if (identical(marker_type, "Selected cutoff")) 0.6 else 0.45,
          show.legend = TRUE
        )
    }
    plot_obj +
