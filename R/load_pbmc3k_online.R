...
#' @param install_if_missing Whether to install the IFNB dataset with
#'   `SeuratData::InstallData("ifnb")` when SeuratData is available but the
#'   dataset is missing. Defaults to [interactive()].
...
#'
#' @details
#' SeuratData is used as an optional dependency. The IFNB dataset can be
#' installed on request with `install_if_missing = TRUE`; this is never done
#' during package checks unless users explicitly call the function that way.
#'
#' @examples
#' \dontrun{
...
load_ifnb_online <- function(add_mito = TRUE,
                             ...
                             sample_col = "sample",
                             group_col = "condition",
                             install_if_missing = interactive(),
                             verbose = TRUE) {
  .ensure_seurat_available()
  if (isTRUE(verbose)) {
  ...
  seurat_obj <- .load_seuratdata_dataset(
    ...
    missing_package_message = paste0(
      ...
      "remotes::install_github('satijalab/seurat-data'), then rerun this function."
    ),
    missing_dataset_message = "The ifnb dataset is not installed. Run SeuratData::InstallData('ifnb') or call load_ifnb_online(install_if_missing = TRUE).",
    install_if_missing = install_if_missing,
    verbose = verbose
  )
  seurat_obj <- .update_seurat_object_if_available(seurat_obj)
...
.update_seurat_object_if_available <- function(seurat_obj) {
  if (!"UpdateSeuratObject" %in% getNamespaceExports("Seurat")) {
    return(seurat_obj)
  }
  tryCatch(
    suppressWarnings(Seurat::UpdateSeuratObject(seurat_obj, verbose = FALSE)),
...
.add_or_normalize_mito_ratio <- function(seurat_obj,
  ...
  mito_features <- .detect_mito_features_robust(seurat_obj, species = species)
  ...
  seurat_obj@meta.data[[mito_col]] <- percent_values / 100
  ...
  seurat_obj@misc$SCdetMito$mito_ratio <- list(
    ...
    species = species,
    pattern = "robust_online_loader_fallback",
    mito_column = mito_col,
    assay = assay,
    scale = "fraction",
    dataset_label = dataset_label,
    fallback_reason = conditionMessage(add_result)
  )
  seurat_obj
}

.detect_mito_features_robust <- function(seurat_obj, species) {
  assay <- Seurat::DefaultAssay(seurat_obj)
  feature_names <- rownames(seurat_obj[[assay]])
  direct_matches <- .match_mito_feature_names(feature_names, species = species)
  if (length(direct_matches)) {
    return(direct_matches)
  }

  meta_matches <- .match_mito_features_from_metadata(
    seurat_obj = seurat_obj,
    assay = assay,
    feature_names = feature_names,
    species = species
  )
  if (length(meta_matches)) {
    return(meta_matches)
  }

  stop(
    "No mitochondrial features were detected by the online loader fallback. ",
    "Number of features: ",
    ...
    ". Species: ",
    species,
    ". Please manually provide mitochondrial features or check whether feature names are Ensembl IDs.",
    call. = FALSE
  )
}

.match_mito_feature_names <- function(feature_names, species) {
  if (!length(feature_names)) {
    return(character(0))
  }

  if (identical(species, "human")) {
    exact_mt <- grep("^MT-", feature_names, value = TRUE, ignore.case = FALSE)
    if (length(exact_mt)) {
      return(unique(exact_mt))
    }
    insensitive_mt <- grep("^MT-", feature_names, value = TRUE, ignore.case = TRUE)
    if (length(insensitive_mt)) {
      return(unique(insensitive_mt))
    }
  }

  patterns <- if (identical(species, "mouse")) {
    c("^mt-", "^Mt-", "^MT-", "^MT\\.", "^MT_", "^mitochondrial")
  } else {
    c("^MT\\.", "^MT_", "^mitochondrial")
  }
  matched <- unique(unlist(lapply(patterns, function(pattern) {
    grep(pattern, feature_names, value = TRUE, ignore.case = TRUE)
  }), use.names = FALSE))
  matched
}

.match_mito_features_from_metadata <- function(seurat_obj,
                                               assay,
                                               feature_names,
                                               species) {
  assay_obj <- seurat_obj[[assay]]
  meta_features <- tryCatch(assay_obj@meta.features, error = function(e) NULL)
  ...
  for (column in candidate_columns) {
    values <- as.character(meta_features[[column]])
    matched_values <- .match_mito_feature_names(values, species = species)
    if (!length(matched_values)) {
      next
    }
    matched_index <- values %in% matched_values
    matched_features <- feature_names[matched_index]
