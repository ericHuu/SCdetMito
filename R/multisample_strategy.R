...
derive_strategy_level_cutoff <- function(sample_plan,
                                         cutoff_column,
                                         cutoff_strategy,
                                         cutoff_support_fraction,
                                         group_by = NULL) {
  if (is.null(sample_plan) ||
    !nrow(sample_plan) ||
    is.null(cutoff_column) ||
    !cutoff_column %in% colnames(sample_plan)) {
    return(NA_real_)
  }

  cutoffs <- as.numeric(sample_plan[[cutoff_column]])
  if (!any(is.finite(cutoffs))) {
    return(NA_real_)
  }

  if (identical(cutoff_strategy, "strictest")) {
    return(min(cutoffs, na.rm = TRUE))
  }

  if (identical(cutoff_strategy, "groupwise") && !is.null(group_by)) {
    group_cutoffs <- tapply(cutoffs, sample_plan$group_id, function(group_cutoffs) {
      resolve_supported_cutoff(group_cutoffs, cutoff_support_fraction)$cutoff
    })
    return(max(as.numeric(group_cutoffs), na.rm = TRUE))
  }
...
build_multisample_cutoff_plan <- function(seurat_obj,
  ...
  selected_column <- if ("selected_cutoff" %in% colnames(sample_plan)) {
    "selected_cutoff"
  } else if ("detected_cutoff" %in% colnames(sample_plan)) {
    "detected_cutoff"
  } else {
    NULL
  }
  recommended_column <- if ("recommended_cutoff" %in% colnames(sample_plan)) {
    "recommended_cutoff"
  } else {
    selected_column
  }
  final_selected_cutoff <- derive_strategy_level_cutoff(
    sample_plan = sample_plan,
    cutoff_column = selected_column,
    cutoff_strategy = cutoff_strategy,
    cutoff_support_fraction = cutoff_support_fraction,
    group_by = group_by
  )
  final_recommended_cutoff <- derive_strategy_level_cutoff(
    sample_plan = sample_plan,
    cutoff_column = recommended_column,
    cutoff_strategy = cutoff_strategy,
    cutoff_support_fraction = cutoff_support_fraction,
    group_by = group_by
  )
  cutoff_applied_source <- paste(unique(sample_plan$cutoff_applied_source), collapse = ";")
  if (identical(cutoff_applied_source, "recommended_cutoff")) {
    final_recommended_cutoff <- final_cutoff
  }
  if (identical(cutoff_applied_source, "selected_cutoff") ||
    identical(cutoff_applied_source, "user_defined")) {
    final_selected_cutoff <- final_cutoff
  }

  list(
    sample_plan = sample_plan,
    sample_cutoff_summary = sample_plan,
    group_plan = group_plan,
    group_cutoff_summary = group_plan,
    final_cutoff = final_cutoff,
    final_selected_cutoff = final_selected_cutoff,
    final_recommended_cutoff = final_recommended_cutoff,
    cutoff_applied = final_cutoff,
