...
test_that("ensure_mito_ratio validates an existing mitochondrial fraction column", {
  seu <- load_demo_seurat()
  updated <- ensure_mito_ratio(seu, mito_col = "mitoRatio")

  expect_true("mitoRatio" %in% colnames(updated@meta.data))
  expect_true(is.numeric(updated$mitoRatio))
  expect_lte(max(updated$mitoRatio, na.rm = TRUE), 1)
  expect_true(is.list(updated@misc$SCdetMito_mito_ratio_info))
  expect_equal(updated@misc$SCdetMito_mito_ratio_info$scale, "fraction")
})

test_that("ensure_mito_ratio converts percent-scale mitochondrial columns", {
  seu <- load_demo_seurat()
  seu$mitoPercent <- seu$mitoRatio * 100

  updated <- ensure_mito_ratio(seu, mito_col = "mitoPercent", convert_percent = TRUE)

  expect_lte(max(updated$mitoPercent, na.rm = TRUE), 1)
  expect_true(updated@misc$SCdetMito_mito_ratio_info$converted_from_percent)
})

test_that("ensure_mito_ratio detects representative human mitochondrial genes in a toy object", {
  counts <- matrix(
    c(10, 5, 20, 15, 10, 5, 30, 15, 90, 80, 100, 110),
    nrow = 6,
    byrow = TRUE
  )
  rownames(counts) <- c("MT-ND1", "MT-CO1", "MT-CO2", "MT-ATP6", "GeneA", "GeneB")
  colnames(counts) <- c("Cell1", "Cell2")
  seu <- suppressWarnings(Seurat::CreateSeuratObject(counts = counts, min.cells = 0, min.features = 0))

  updated <- ensure_mito_ratio(seu, species = "human", mito_col = "mitoRatio")

  expect_true("mitoRatio" %in% colnames(updated@meta.data))
  expect_equal(updated@misc$SCdetMito_mito_ratio_info$n_mito_features, 4L)
})

test_that("exact mitochondrial fallback presets include rRNA features", {
  expect_true(all(c("MT-RNR1", "MT-RNR2") %in% mito_species_exact_features("human")))
  expect_true(all(c("mt-Rnr1", "mt-Rnr2") %in% mito_species_exact_features("mouse")))
})
...
test_that("SCdetMito auto_add_mito calculates a missing mitochondrial column", {
  seu <- make_test_seurat()
  seu$mitoRatio <- NULL
  ...
  expect_true("detected_cutoff" %in% names(det$sample_cutoff_summary))
})

test_that("SCdetMito recompute_mito updates an existing mitochondrial column only when requested", {
  seu <- make_test_seurat()
  original <- seu$mitoRatio
  seu$mitoRatio <- 0
  ...
  expect_true(any(preserved$retained_cell_profile$retention_fraction == 1))
  expect_true(any(recomputed$retained_cell_profile$retention_fraction < 1))
  recalculated <- ensure_mito_ratio(seu, species = "human", overwrite = TRUE, verbose = FALSE)
  expect_equal(
    max(abs(recalculated$mitoRatio - original), na.rm = TRUE),
    0
  )
})

test_that("existing and auto-computed demo mitoRatio give consistent cutoffs", {
  seu <- load_demo_seurat()
  det_existing <- SCdetMito(
    seu,
    sample_col = "sample",
    mito_col = "mitoRatio",
    species = "human",
  ...
  seu2 <- load_demo_seurat()
  existing_values <- seu2$mitoRatio
  seu2$mitoRatio <- NULL
  auto_object <- ensure_mito_ratio(seu2, mito_col = "mitoRatio", species = "human", verbose = FALSE)
  det_auto <- SCdetMito(
    seu2,
    sample_col = "sample",
    mito_col = "mitoRatio",
  ...
  expect_equal(auto_object$mitoRatio, existing_values, tolerance = 1e-8)
  expect_equal(
    det_existing$sample_cutoff_summary$selected_cutoff,
    det_auto$sample_cutoff_summary$selected_cutoff,
    tolerance = 1e-8
  )
  expect_equal(
    det_existing$sample_cutoff_summary$detected_cutoff,
    det_existing$sample_cutoff_summary$selected_cutoff
  )
})

test_that("SCdetMito errors clearly when auto_add_mito is disabled and mito_col is missing", {
  seu <- make_test_seurat()
  seu$mitoRatio <- NULL
...
test_that("add_mitoRatio remains a backward-compatible wrapper", {
  seu <- make_test_seurat()
  seu$mitoRatio <- NULL
...
test_that("public exports include intended wrappers and exclude dot-prefixed helpers", {
  ...
  expect_false("load_ifnb_online" %in% exports)
  expect_false(any(grepl("^\\.", exports)))
})
...
test_that("SCdetMito exports interval-loss tables and legacy aliases", {
  ...
  expect_true(file.exists(file.path(output_dir, "SCdetMito_recommendation_summary.csv")))
  expect_true(file.exists(file.path(output_dir, "SCdetMito_reference_cutoff_summary.csv")))
...
test_that("detected_cutoff is backward-compatible alias of selected_cutoff", {
  ...
  expect_false(any(is.na(summary$recommended_cutoff)))
  expect_equal(summary$detected_cutoff, summary$selected_cutoff)
})
...
test_that("reference-aware cutoff modes report first-boundary and selected cutoffs", {
  ...
  expect_equal(strict$settings$auto_apply_max_reference_ratio, 2)
  expect_equal(strict$settings$extreme_reference_ratio, 3)
})
